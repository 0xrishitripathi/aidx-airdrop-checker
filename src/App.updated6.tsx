...
function App() {
  ...
  const [finalEvmAddressForAirdrop, setFinalEvmAddressForAirdrop] = useState('');
  ...
  const canRegister = () => {
    // If registration was successful, don't show the button
  ...
  const checkEligibility = async (address: string) => {
    try {
      ...
      if (registrationData.isRegistered) {
        ...
        setFinalEvmAddressForAirdrop(registrationData.registeredWallet.final_evm_address);
  ...
  const disconnectAvailWallet = () => {
    ...
    setIsAlreadyRegistered(false);
    // Reset registration status if both wallets are disconnected
    if (!evmAddress) {
      setRegistrationSuccessful(false);
      setFinalEvmAddressForAirdrop('');
    }
  };
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          setLinkingStatus('');
          setRegistrationSuccessful(true);
          setFinalEvmAddressForAirdrop(finalEvmAddress);
  ...
  return (
    <div className="app">
      ...
      <div className="login-container">
        ...
        {/* Centralized Registration Button or Success Message */}
        {registrationSuccessful ? (
          <div className="registration-success">
            <p className="success-message">You will receive the AIDX tokens on <span className="highlighted-address">{truncateAddress(finalEvmAddressForAirdrop)}</span></p>
            <p className="airdrop-address">✨ You've successfully registered for the airdrop! 🥳</p>
          </div>
        ) : canRegister() && (
